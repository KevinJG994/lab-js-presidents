...
function getNames(presidentsArr) {
  const allPresidents = presidentsArr.map((presidents) => presidents.name);
  return allPresidents;
}

// Iteration 2 | Democratic Presidents - `filter()`
function getDemocraticPresidents(presidentsArr) {
  const democraticPresident = presidentsArr.filter(
...
function getPresidentsBornAfter(presidentsArr, year) {
  const presidentsAfterAge = presidentsArr.filter(
    (president) => president.birthYear > year
  );
  return presidentsAfterAge;
}

// Bonus: Iteration 7 | Count Republican Presidents
function countRepublicanPresidents(presidentsArr) {
  const totalRepublicans = presidentsArr.reduce((acc, president) => {
